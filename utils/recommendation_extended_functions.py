...
def get_extended_course_recommendations(user_input, no_of_recommendations):
    ...
    course_data['extended_description'] = course_data['extended_description'].str.lower()
    ...
    course_data['extended_description'] = course_data['extended_description'].apply(lambda x: x.replace("-", " "))
    ...
    course_data['extended_description'] = course_data['extended_description'].apply(lambda x: x.translate(str.maketrans('', '', string.punctuation)))
    ...
    course_data['extended_description'] = course_data['extended_description'].apply(lambda x: [lemmatizer.lemmatize(word.lower()) for word in word_tokenize(x)])
    ...
    course_data['extended_combined_text'] = course_data['skills'] + course_data['skill_groups'] + course_data['extended_description'] + course_data['category'] + course_data['course_name']

    # Combine text attributes
    course_data['extended_combined_text'] = course_data['extended_combined_text'].apply(lambda x: ' '.join(x))

    # Vectorize text data
    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform(course_data['extended_combined_text'])
...
# Sample usage:
get_extended_course_recommendations("Launch marketing campaign for food-tech mobile application", 5)
